...
1. **Repository context**
   - Project lives at `/Volumes/DatenAP/Code/mcp-server`.
...
4. **Deployment target**
   - Built files go to `/var/www/mcp.arkturian.com`.
   - Backups are stored with prefix `mcp-server` under `/var/backups/` (configurable via templates).

5. **Startup checklist for new session**
   - `cd /Volumes/DatenAP/Code/mcp-server`.
